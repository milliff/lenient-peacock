...
#
# Script Description: U.S. approval analysis using IIOPO, Gallup, and Pew data. Part 4 of the Element.
#
# Produces: fig4-1.png through fig4-4.png
#
# --------------------------------------------
...
time_df_long_us <- time_df_natl |> filter(country == "USA")

timeplot_natl <- ggplot(time_df_long_us) + geom_hline(yintercept = 0, color = "darkgrey") +
  geom_smooth(aes(x = year, y = approval), se = F, span = .2, lwd=1.1, color = ptol_pal()(2)[1])  +
  geom_point(aes(x = year, y = approval), pch = 20, alpha = .7, color = ptol_pal()(2)[1]) +
  geom_vline(xintercept = as.POSIXct("1971-12-18 UTC"), lty = 2, color = "grey") +
...
time_df_natl_us <- time_df_natl |> filter(country == "USA")
...
time_df_natl_pew <- dfall |> group_by(YEAR) |> 
  ...
  pivot_longer(!c(YEAR),
               names_to = "country",
               values_to = "approval") |> 
  drop_na() |> filter(country == "USA") |> select(-country)

time_df_natl_us <- time_df_natl_us |> select(-country) |> rename(YEAR = YEAR_WAVE)

time_df_natl_pew$source <- "Pew Global Attitudes\n(Country)"
time_df_natl_us$source <- "Gallup World Poll\n(Leadership)"

pew_gallup_plot <- rbind.data.frame(time_df_natl_pew, time_df_natl_us)
...
# Structure of Opinion Stuff -------------------------------------------------------------------------
# Structure of Opinions Stuff ---------------------------------------------

# Pew
dfall$region <- relevel(factor(dfall$region), ref = "North")
pew_pooled <- lm_robust(approve_us_bin ~ region + religion + edu + income + male + age, data= dfall, fixed_effects = ~YEAR)
pew_pooled

pew_party <- lm_robust(approve_us_bin ~ region + religion + edu + income + male + age +partyid, data= dfall, fixed_effects = ~YEAR)
pew_party
# Gallup

gallup_pooled <- lm_robust(us_leader_bin ~ female + age + educ  + percap_income_localcurrency + region, data= indf, fixed_effects = ~YEAR_WAVE)
gallup_pooled


# IIPO
df$city <- relevel(factor(df$city), ref = "Delhi")
iipo_pooled <- lm_robust(usa_bin ~ city + sex + age + educ  + income + religion, data = df, fixed_effects = ~ wave)
iipo_pooled
...
regionplot <- modelsummary::modelplot(list("Pew (2002-2018)" = pew_pooled,
  ...
  scale_color_ptol(0) +
  labs(title = "Regional Variation in Approval of United States",
...
ggsave(filename = here::here("results/figs/fig4-3.png"),plot = regionplot, width = 8, height = 5, units = "in", dpi = "retina")
...
party_in_power <- lm_robust(approve_us_bin ~ bjp*ndagov + region + religion + male + age + edu + income, data = dfall)

threeparty_in_power <- lm_robust(approve_us_bin ~ ndagov*threeparty + region + religion + male + age + edu + income, data = dfall)
...
pew_partymod <- lm_robust(approve_us_bin ~ YEAR*bjp*region + religion + male + age + edu + income, data = dfall)
...
big_plot_byregion <- ggplot(big_party_region_plot, aes(x = year, y = pp, group = region, color = region)) + 
  ...
  labs(title = "Predicted Favorability Toward USA",
       ...
       x = "Year",
       y = "p(Favorable)",
       color = "Region")+ ylim(c(0,1))

ggsave(filename = here::here("results/figs/fig4-4.png"),plot = big_plot_byregion, width = 8, height = 5, units = "in", dpi = "retina")
